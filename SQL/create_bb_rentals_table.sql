...
CREATE OR REPLACE TABLE
  `skyuk-uk-csgbillanalysis-dev.roi_rental.roi_apr_22_bb_rentals` AS /* NEED TO CHANGE THIS INVOICE */
WITH
  INVOICE AS (
  SELECT
  ...
  BILL_VALUE AS (
  SELECT
    Reference_CLI AS Reference_CLI2,
...
SELECT
  ...
  ORDER_ID,
  ORDER_TYPE_CODE,
  DATE(effective_to_dt) AS EFFECTIVE_TO_DATE,
  offer_status,
  portfolioid,
  serviceinstanceid,
  DESCRIPTION,
...
WHERE
  (LATEST_SERVICE = 1 OR LATEST_SERVICE IS NULL)
GROUP BY
  ACCOUNT_NUMBER,
  TYPE,
  bo_account_number,
